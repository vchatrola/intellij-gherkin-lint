...
The project is a standard IntelliJ plugin with a few focused modules:
...
- **Prompt building and rules**: JSON rules define validation behavior, and prompt builders turn those rules into a Gemini prompt.
- **Gemini integration**: a small Spring-based client handles the REST calls to Gemini’s API.
- **Output parsing and display**: the JSON response is parsed and printed in a readable, styled console view.
- **Settings UI**: users can enable custom rule files and copy a sample rules template.
...
5. `GeminiService` sends the prompt to Gemini using a REST client.
...
- `src/main/java/com/vchatrola/gemini/config/AppConfig.java`: Spring config and REST client setup for Gemini.
- `src/main/java/com/vchatrola/gemini/service/GeminiService.java`: request/response wrapper for Gemini calls.
- `src/main/java/com/vchatrola/prompt/PromptBuilder.java`: builds the final prompt from JSON rules.
- `src/main/java/com/vchatrola/config/ConfigurationManager.java`: loads and merges default/custom rules.
...
- The Gemini API key is provided via environment variable, not hard‑coded.
